...
glmGamPoi <- function(matrix,gene_name,cell_name){
    ...
    rna <- CreateSeuratObject(counts = t(matrix),project = "mix_rna",min.cells = 10)

    rna <- SCTransform(rna,method = "glmGamPoi",verbose = FALSE)

    normalize_count <- as.data.frame(rna@assays$SCT@data)

    scale_count <- as.data.frame(rna@assays$SCT@scale.data)

    sctransform_gene_list <- rna@assays$SCT@var.features

    return(sctransform_gene_list)
