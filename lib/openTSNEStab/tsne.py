...
class TSNE(BaseEstimator):
    ...
    def fit(self, X=None, remain=None, affinities=None, initialization=None):
        ...
        ## 随机采样
        sample_p = 1.0 #采样率
        anchors = random.sample(remain.tolist(),int(sample_p*len(remain)))
        anchors.sort()

        embedding = self.prepare_initial(X, remain, anchors, affinities, initialization)
    ...
    def prepare_initial(self, X=None, remain=None, anchors=None, affinities=None, initialization=None):
...
def kl_divergence_bh(
    ...
    **_,
    ...
    # logger = logger_config(log_path='log.txt', logging_name='bh')  #日志生成器
...
def kl_divergence_fft(
    ...
    **_,
    ...
    # logger = logger_config(log_path='log.txt', logging_name='fft') #日志生成器
...
class gradient_descent:
    ...
    def __call__(
        ...
        # logger = logger_config(log_path='log.txt', logging_name='gradient_descent')
        ...
        for iteration in range(n_iter):
            ...
            alpha = 0.5
            ## 原始项
            error1, gradient1 = objective_function(
                embedding,
                P,
                dof=dof,
                bh_params=bh_params,
            ...
            # 稳定项
            nor_anchors = list(set(remain) - set(anchors))
            embedding_part = embedding.copy()
            embedding_part[nor_anchors] = 0
            P_part = P.copy()
            P_part[nor_anchors] = 0
            error2, gradient2 = objective_function(
                embedding_part,
                P_part,
                dof=dof,
                bh_params=bh_params,
            ...
            error = (1-alpha) * error1 + alpha * error2
            gradient = (1-alpha) * gradient1 + alpha * gradient2           

            # embedding_part = embedding[anchors].copy()
            # P_part = P[anchors].copy()
            # error2, gradient2 = objective_function(
            #     embedding_part,
            #     P_part,
            #     dof=dof,
            ...
            # error = (1-alpha) * error1 + alpha * error2
            # gradient = gradient1
            # gradient[anchors] = (1-alpha) * gradient1[anchors] + alpha * gradient2
